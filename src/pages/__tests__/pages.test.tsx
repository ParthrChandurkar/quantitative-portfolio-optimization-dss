...
const analytics:AnalyticsBundle={methodology:{label:'OUT-OF-SAMPLE BACKTEST',estimation_start_date:'2024-01-01',estimation_last_date:'2024-12-31',split_date:'2025-01-01',evaluation_start_date:'2025-01-01',evaluation_end_date:'2025-12-31',estimation_observations:252,evaluation_observations:249,windows_overlap:false},allocation:[{symbol:'RELIANCE',sector:'Energy',weight:1,allocated_amount_inr:1_000_000}],risk_return:{expected_return:.16,volatility:.2},growth_projection:[],performance:{buy_and_hold:{points:[{trade_date:'2025-01-01',portfolio_value:1_000_000,portfolio_return:0},{trade_date:'2025-12-31',portfolio_value:1_120_000,portfolio_return:.12}],warnings:[],validation_mode:'out_of_sample',estimation_end_date:'2025-01-01'},periodic_rebalance:{points:[{trade_date:'2025-01-01',portfolio_value:1_000_000,portfolio_return:0},{trade_date:'2025-12-31',portfolio_value:1_100_000,portfolio_return:.1}],warnings:[],validation_mode:'out_of_sample',estimation_end_date:'2025-01-01'}},risk_metrics:{sharpe_ratio:.8,model_annualized_volatility:.2,realized_annualized_volatility:.18,max_drawdown:-.08,parametric_var_95:169000,historical_var_95:43000,realized_annualized_return:.10,realized_sharpe_ratio:.44},efficient_frontier:[{expected_return:.12,volatility:.15}],sector_distribution:[{sector:'Energy',allocation:1,cap:1,remaining_capacity:0,is_binding:true,exceeds_cap:false}]}
...
describe('real-client mutation flows',()=>{
  it('labels analytics as a non-overlapping out-of-sample backtest',async()=>{saveSelection({portfolioId:'portfolio-1',snapshotId:'snapshot-1'});show(<AnalyticsPage/>,api());expect((await screen.findAllByText('OUT-OF-SAMPLE BACKTEST')).length).toBeGreaterThan(0);expect(screen.getByText('Verified: zero overlapping dates')).toBeInTheDocument()})
