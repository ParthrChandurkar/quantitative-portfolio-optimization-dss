...
export function Shell() {
  ...
  </aside>{open&&<div className="backdrop" onClick={()=>setOpen(false)}/>}<main><header><button aria-label="Open menu" className="menu" onClick={()=>setOpen(true)}><Menu/></button><div><span className="crumb">PORTFOLIO INTELLIGENCE</span><h1>{current}</h1></div><div className="top-actions"><div className="market"><span className="pulse"/>LIVE BACKEND <b>NIFTY 50</b></div></div></header><div className="content"><Outlet/></div></main></div>
}
